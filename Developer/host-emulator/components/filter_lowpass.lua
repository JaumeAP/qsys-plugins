...
-- index of Q-SYS Help's filter_lowpass.htm page states the component
...
-- SubharmonicSynth's own GetWiring, which wires it as "Lpf Input 1"/
-- "Lpf Output 1" (previously the only evidence for this file).
-- Stereo/Multi-Channel pin naming is NOT modeled -- no plugin in this repo
-- uses filter_lowpass outside Mono, so extending this file for an untested
-- channel count would be speculative; revisit if that changes.
