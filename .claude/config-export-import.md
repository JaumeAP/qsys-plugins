...
2. **Import** (a config bundle uploaded into a session — "reverse
   ...
   2.2. **Mandatory core files**: every hook under `hooks/`, plus
        ...
        means "ask before installing," not "fetch it live"). This file
        used to also embed its full current `SKILL.md` content verbatim
        in its own appendix, so a repo without it installed still had the
        complete definition on hand locally — removed 2026-07-31 (explicit
        user request): the user now holds their own extracted copy outside
        this repo and will hand it back when they want it reinstalled, so
        the appendix stopped pulling its weight. `.claude/skills-history.md`
        has the full timeline with reasoning at each turn.
   ...
   2.5. **Bundled-file skills offered as an optional choice**: the
        ...
        it reflexively just because it's offered. Its full current
        `SKILL.md` content is no longer embedded in this file (the
        appendix that used to carry it was removed 2026-07-31, explicit
        user request — see the history note below) — if a target repo
        declines it or the skill is never asked about, get the content
        from `.claude/skills/changelog-rules/` in a repo that already has
        it installed, or ask the user for their own extracted copy.
        `file-operations` and `github-rules` are NOT in this group — both
        are mandatory blind-copies, see 2.2.
