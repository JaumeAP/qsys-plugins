...
# after -- see config-export-import.md step 2.2); changelog-rules and
# find-skills are the
# optional import choices (step 2.5) -- changelog-rules briefly removed from
...
# explicit user request) rather than staying deleted; find-skills took a
# longer road the same general period -- optional, then deleted
...
# resting state, at least so far. Each skill has its
...
- `references/skills/<name>/<name>.md` -- four generic skills.
  ...
  after). `changelog-rules` and `find-skills` are the
  optional import choices (step 2.5). `find-skills` had a longer round
  trip the same general period
  ...
  being: optional, alongside `changelog-rules`, not mandatory.
  `changelog-rules` itself was briefly removed
