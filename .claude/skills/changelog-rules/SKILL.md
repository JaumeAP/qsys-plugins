...
- Do not write changelog entries to disk mid-session; write the accumulated
  entry just before each `git commit`, staged into that same commit
...
2. **Update file:** Add entry, maintain chronological order (newest first), stage it into the same commit
...
- **v1.1.2** (2026-07-31) - Fixed self-contradiction in commit/push timing; closed taxonomy opened to match real usage
  - bugfix: "Accumulation"/"On Push" said write just before `git push`, contradicting `CLAUDE.md`'s own
  ...
  - bugfix: `Types` was a closed 7-item enum with ~2.4% real-world compliance (295-entry audit); added
